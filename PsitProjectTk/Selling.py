...
def basket_show():
    ...
    if basket == None:
        ...
        for data in basket:
            number_of_pro += 1
            check_as = "SELECT aftersold FROM products WHERE productsId = " + str(data[0])
            ...
            after = "        จำนวนสินค้าในคลัง :      %d"%(after_sold_from_pro[0][0])
...
def set_basket():
    ...
    if "รหัสสินค้า" not in text:
        messagebox.showerror("Error", "กรุณาคลิกเลือกรหัสสินค้าก่อนเพิ่มลงตะกร้า")
    elif quantity_entry.get().isnumeric() == False:
        messagebox.showerror("Error", "กรุณาใส่จำนวนสินค้าให้ถูกต้อง")
    else:
        text = text[27:]
        stock_check = "SELECT afterSold FROM products WHERE productsId = '" + text +"'"
        mycursor.execute(stock_check)
        pro_in_stock = mycursor.fetchall()
        stock_check = "SELECT quantityPro FROM basket WHERE productId = '" + text +"'"
        mycursor.execute(stock_check)
        quantity_in_basket = mycursor.fetchall()
        qtt_of_this_pro = 0
        for qtt in quantity_in_basket:
            qtt_of_this_pro += qtt[0]
        if (int(quantity_entry.get()) > pro_in_stock[0][0]) or ((int(quantity_entry.get())+qtt_of_this_pro) > pro_in_stock[0][0]):
            ...
            for id_bas in id_basket:
                if id_bas[0] == int(text):
                    sql = "UPDATE basket SET quantityPro = quantityPro + " + quantity_entry.get() + ""
                ...
                sql = "INSERT INTO basket (productId, productName, category, sell, afterSold, quantityPro) VALUES (%s, %s, %s,%s ,%s, %s)"
                val = (data_to_basket[0][0], data_to_basket[0][1], data_to_basket[0][2] ,data_to_basket[0][4], data_to_basket[0][6], int(quantity_entry.get()))
...
def finish_sell():
    ...
    if basket == []:
        ...
        for data in basket:
            # print(data)
            sql = "UPDATE products SET afterSold = afterSold - ' " + str(data[5])+ "'WHERE productsId= '" + str(data[0]) + "'"
            mycursor.execute(sql)
            mydb.commit()
...
def delete_from_basket():
    ...
    if "รหัสสินค้า" not in del_text:
        ...
        response = messagebox.askokcancel("Confirm", "ต้องการลบสินค้าชิ้นนี้จากตะกร้าหรือไม่ ?")
        if response == 1:
