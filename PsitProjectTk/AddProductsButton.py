...
def product():
    ...
    Label(product_win, text='จัดการสินค้า', font=("TH Sarabun New", 25+10),
          width=10, padx=1, pady=5).grid(row=0, column=1, columnspan=5)

    Label(product_win, text='ชื่อสินค้า', font=("TH Sarabun New", 16+10),
          width=10, anchor="e").grid(row=1, column=1, columnspan=1)
    name = Entry(product_win, width=12, font=("TH Sarabun New", 14+10),
                 bg='gray99')
    name.grid(row=1, column=2)

    Label(product_win, text='ประเภทสินค้า', font=("TH Sarabun New", 16+10),
          width=12, pady=10, anchor="e").grid(row=1, column=3)

    def pro_id_def():
        """Assign Products ID"""
        global pro_id
        pro_id = 0
        check_id = "SELECT * FROM products WHERE productsID=\
            (SELECT MAX(productsId) FROM products)"
    ...
    def category_show():
        ...
        category.configure(width=15, justify=CENTER, font=("TH Sarabun New",
                           15+10), bg="gray30", fg="white", borderwidth=0)
        category['menu'].config(font=("TH Sarabun New", (15+10)),
                                bg="gray30", fg="white")
    ...
    def add_category():
        ...
        Label(add_cate, text="เพิ่มประเภทสินค้า",
              font=("TH Sarabun New", 15+10)).pack(side=LEFT)
        global new_cate
        new_cate = Entry(add_cate, width=20, font=("TH Sarabun New", 15+10))
        new_cate.pack(side=LEFT)
        submit_add_cate = Button(add_cate, text='ยืนยัน',
                                 command=update_category,
                                 font=("TH Sarabun New", 15+10)).pack()

    def edit_cate_to_db(text):
        """Edit category to Database"""
        sql = "UPDATE products SET category = '" + e_cate.get() + \
            "'WHERE category= '" + text + "'"
    ...
    def edit_cate_command():
        ...
        if listbox.get(ACTIVE) == "คลิกเลือกประเภทสินค้าที่จะแก้ไข  ":
            ...
            global e_cate
            Label(edit_cate, text="แก้ไขประเภทสินค้า",
                  font=("TH Sarabun New", 15+10)).pack(side=LEFT)
            e_cate = Entry(edit_cate, width=20, font=("TH Sarabun New", 15+10))
            text = (listbox.get(ACTIVE)).lstrip(" ")
            e_cate.insert(0, text)
            e_cate.pack(side=LEFT)
            submit_edit_cate = Button(edit_cate, text='ยืนยัน',
                                      command=lambda: edit_cate_to_db(text,),
                                      font=("TH Sarabun New", 15+10)).pack()
    ...
    def edit_catagory():
        ...
        edit_show.geometry("800x600")
        scrollbar = Scrollbar(edit_show)
        scrollbar.pack(side=LEFT, fill=Y)

        global listbox
        listbox = Listbox(edit_show, yscrollcommand=scrollbar.set,
        ...
        Button(edit_show, text="แก้ไขประเภทสินค้า", command=edit_cate_command,
               font=("TH Sarabun New", 14+10), bg='white', relief="raised",
               width=20, padx=1).pack()
    ...
    category_add_btn = Button(product_win, text="เพิ่ม", command=add_category,
                              font=("TH Sarabun New", 14+10), relief="raised",
                              width=7, padx=1).grid(row=1, column=5,
                                                    sticky='w')
    category_edit_btn = Button(product_win, text="แก้ไข",
                               command=edit_catagory,
                               font=("TH Sarabun New", 14+10), relief="raised",
                               width=7, padx=1).grid(row=1, column=6,
                                                     sticky='e')

    Label(product_win, text='ราคาซื้อ', font=("TH Sarabun New", 16+10),
          width=10, anchor="e").grid(row=2, column=1, columnspan=1)
    buy = Entry(product_win, width=12, font=("TH Sarabun New", 14+10),
                bg='gray99')
    buy.grid(row=2, column=2)
    Label(product_win, text='ราคาขาย', font=("TH Sarabun New", 16+10),
          width=12, anchor="e").grid(row=2, column=3, columnspan=1)
    sell = Entry(product_win, width=15, font=("TH Sarabun New", 14+10),
                 bg='gray99')
    ...
    def edit_pro_to_db(text):
        ...
        if "รหัสสินค้า" in text:
            ...
            for data in product_data:
                if int(e_pro.get()) == data[0]:
                    messagebox.showerror("Error",
                                         "มีรหัสสินค้านี้ในระบบอยู่แล้ว")
                    break
            else:
                text = text[27:]
                sql = "UPDATE products SET productsId = '" + e_pro.get() + \
                    "'WHERE productsId= '" + text + "'"
                mycursor.execute(sql)
                mydb.commit()
        elif "ชื่อสินค้า" in text:
            text = text[29:]
            sql = "UPDATE products SET productsName = '" + e_pro.get() + \
                "'WHERE productsName= '" + text + "'"
            mycursor.execute(sql)
            mydb.commit()
        elif "ประเภทสินค้า" in text:
            text = text[26:]
            sql = "UPDATE products SET category = '" + e_pro.get() + \
                "'WHERE category= '" + text + "'"
            mycursor.execute(sql)
            mydb.commit()
            ...
            sql = "UPDATE products SET buy = '" + e_pro.get() + \
                "'WHERE buy= '" + text + "'"
            mycursor.execute(sql)
            mydb.commit()
            ...
            text = text[26:]
            text = text.rstrip(" บาท")
            sql = "UPDATE products SET sell = '" + e_pro.get() + \
                "'WHERE sell= '" + text + "'"
            mycursor.execute(sql)
            mydb.commit()
    ...
    def edit_pro_command():
        ...
        if "สินค้าชิ้นที่  1" in text:
            messagebox.showerror("Error",
                                 "กรุณาคลิกเลือกข้อมูลที่ต้องการแก้ไข")
        else:
            global edit_pro
            edit_pro = Toplevel()
            edit_pro.title("Edit Products")
            global e_pro
            Label(edit_pro, text="แก้ไขสินค้า",
                  font=("TH Sarabun New", 15+10)).pack(side=LEFT)
            e_pro = Entry(edit_pro, width=20, font=("TH Sarabun New", 15+10))
            if "รหัสสินค้า" in text:
                e_pro_id = Entry
            ...
            submit_edit_pro = Button(edit_pro, text='ยืนยัน', command=lambda:
                                     edit_pro_to_db(listbox_pro.get(ACTIVE)),
                                     font=("TH Sarabun New", 15+10)).pack()
    ...
    def delete_pro_command():
        ...
        if "รหัสสินค้า" in text:
            ...
            global e_pro_delete
            Label(delete_pro, text="ระบุรหัสสินค้า",
                  font=("TH Sarabun New", 15+10)).pack(side=LEFT)
            e_pro_delete = Entry(delete_pro, width=20,
                                 font=("TH Sarabun New", 15+10))
            e_pro_delete_id = Entry
            e_pro_delete.insert(0, text[27:])
            e_pro_delete.pack(side=LEFT)
            submit_edit_pro = Button(delete_pro, text='ลบสินค้า',
                                     command=delete_pro_from_db,
                                     font=("TH Sarabun New", 15+10)).pack()
        else:
    ...
    def show_products():
        ...
        products_showing.geometry("800x600")
        scrollbar = Scrollbar(products_showing)
        scrollbar.pack(side=LEFT, fill=Y)

        global listbox_pro
        listbox_pro = Listbox(products_showing, yscrollcommand=scrollbar.set,
                              font=("TH Sarabun New", 14+10))
        ...
        Button(products_showing, text="แก้ไขสินค้า", command=edit_pro_command,
               font=("TH Sarabun New", 14+10), bg='white', relief="raised",
               width=20, padx=1).pack()
        Button(products_showing, text="ลบสินค้า", command=delete_pro_command,
               font=("TH Sarabun New", 14+10), bg='white', relief="raised",
               width=20, padx=1).pack()
    ...
    def add_products():
        ...
        if name.get() == "":
            ...
            sql = "INSERT INTO products (productsId, productsName, category, \
                buy, sell, beforeSold, afterSold) \
                VALUES (%s, %s, %s,%s ,%s, %s, %s)"
            val = (pro_id, name.get(), clicked.get(), buy.get(), sell.get(),
                   0, 0)
            mycursor.execute(sql, val)
            mydb.commit()
            check_id = "SELECT * FROM products WHERE productsID=(SELECT \
                MAX(productsId) FROM products)"
            mycursor.execute(check_id)
            myresult = mycursor.fetchone()
            pro_id == myresult[0]
            pro_id += 1
    ...
    products_show_btn = Button(product_win, text="แสดงสินค้า",
                               command=show_products,
                               font=("TH Sarabun New", 14+10), bg='steelblue2',
                               relief="raised",
                               width=15, padx=1).grid(row=3, column=2,
                                                      columnspan=1)
    products_add_btn = Button(product_win, text="เพิ่มสินค้า",
                              command=add_products,
                              font=("TH Sarabun New", 14+10), bg='goldenrod1',
                              relief="raised",
                              width=21, padx=1).grid(row=3, column=4,
                                                     columnspan=1)
