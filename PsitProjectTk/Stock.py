...
def stock():
    ...
    listbox_stock = Listbox(stock_showing, yscrollcommand=scrollbar.set,
    ...
    Label(stock_showing, text="คลังสินค้า",
          font=("TH Sarabun New", 24+10), width=10, padx=1).pack()

    Button(stock_showing, text="เพิ่มจำนวนสินค้าในคลัง", command=lambda:
           set_stock(True), font=("TH Sarabun New", 14+10), bg='white',
           relief="raised", width=20).pack()
    Button(stock_showing, text="กำหนดจำนวนสินค้าในคลัง", command=lambda:
           set_stock(False), font=("TH Sarabun New", 14+10), bg='white',
           relief="raised", width=20).pack()
...
def stock_set_win_to_db(check_func):
    """set database"""
    if check_func:
        sql = "UPDATE products SET beforesold = afterSold + ' " + \
            stock_quantity_entry.get() + "'WHERE productsId= '" + \
            stock_id_entry.get() + "'"
        ...
        sql = "UPDATE products SET afterSold = ' " + \
            stock_quantity_entry.get() + "'WHERE productsId= '" + \
            stock_id_entry.get() + "'"
        mycursor.execute(sql)
        mydb.commit()
        sql = "UPDATE products SET beforesold = ' " + \
            stock_quantity_entry.get() + "'WHERE productsId= '" + \
            stock_id_entry.get() + "'"
        mycursor.execute(sql)
        mydb.commit()
...
def set_stock(check_func):
    ...
    global stock_entry
    Label(stock_set_win, text="รหัสสินค้า",
          font=("TH Sarabun New", 15+10), anchor="e").grid(row=0, column=0)
    Label(stock_set_win, text="จำนวนที่ต้องการเพิ่ม",
          font=("TH Sarabun New", 15+10), anchor="w").grid(row=1, column=0)
    global stock_id_entry
    stock_id_entry = Entry(stock_set_win, width=20,
    ...
    stock_quantity_entry = Entry(stock_set_win, width=20,
                                 font=("TH Sarabun New", 15+10))
    stock_quantity_entry.grid(row=1, column=1)
    stock_id_entry.grid(row=0, column=1)
    submit_stock_set_id = Button(stock_set_win, text='ยืนยัน', command=lambda:
                                 stock_set_win_to_db(check_func),
                                 font=("TH Sarabun New", 15+10), height=2,
                                 width=6).grid(row=0, column=2, rowspan=2)
