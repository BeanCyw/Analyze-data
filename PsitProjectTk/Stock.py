...
def stock():
    ...
    stock_showing.geometry("500x300+500+150")
    global scrollbar
    scrollbar = Scrollbar(stock_showing)
    scrollbar.pack(side=LEFT, fill=Y)
    global listbox_stock
    listbox_stock = Listbox(stock_showing, yscrollcommand=scrollbar.set,font=("TH Sarabun New", 14))
    ...
    Label(stock_showing,text="คลังสินค้า",font=("TH Sarabun New", 24),width=10,padx=1).pack()

    Button(stock_showing,text="เพิ่มจำนวนสินค้าในคลัง",command=lambda : set_stock(True) ,font=("TH Sarabun New", 14),bg='white',relief="raised",width=20).pack()
    Button(stock_showing,text= "กำหนดจำนวนสินค้าในคลัง",command=lambda : set_stock(False) ,font=("TH Sarabun New", 14),bg='white',relief="raised",width=20).pack()
...
def set_stock(check_func):
    ...
    global stock_entry
    Label(stock_set_win, text="รหัสสินค้า",font=("TH Sarabun New", 15),anchor="e").grid(row=0,column=0)
    Label(stock_set_win, text="จำนวนที่ต้องการเพิ่ม",font=("TH Sarabun New", 15),anchor="w").grid(row=1,column=0)
    global stock_id_entry
    stock_id_entry = Entry(stock_set_win, width=20,font=("TH Sarabun New", 15))
    text = (listbox_stock.get(ACTIVE)).lstrip(" ")
    if "รหัสสินค้า" in text:
        stock_id_entry.insert(0, text[19:])
    global stock_quantity_entry
    stock_quantity_entry = Entry(stock_set_win, width=20,font=("TH Sarabun New", 15))
    stock_quantity_entry.grid(row=1,column=1)
    stock_id_entry.grid(row=0,column=1)
    submit_stock_set_id = Button(stock_set_win, text='ยืนยัน',command=lambda : stock_set_win_to_db(check_func) , font=("TH Sarabun New", 15), height=2, width=6).grid(row=0,column=2,rowspan=2)